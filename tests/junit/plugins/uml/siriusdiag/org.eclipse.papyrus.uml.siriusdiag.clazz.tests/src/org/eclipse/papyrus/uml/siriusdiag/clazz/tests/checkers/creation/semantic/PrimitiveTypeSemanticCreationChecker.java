...
/**
 * Creation checker for UML PrimitiveType
 */
public class PrimitiveTypeSemanticCreationChecker extends AbstractSemanticNodeCreationChecker {
	 ...
	 * @param expectedOwner
	 */
	public PrimitiveTypeSemanticCreationChecker(final EObject expectedOwner) {
		super(expectedOwner);
	}
	...
	protected void validateSemanticElementInstance(final EObject semanticElement) {
		Assert.assertTrue(NLS.bind("The created element must be a UML PrimitiveType instead of a {0}.", semanticElement.eClass().getName()), semanticElement instanceof PrimitiveType); //$NON-NLS-1$
	}
