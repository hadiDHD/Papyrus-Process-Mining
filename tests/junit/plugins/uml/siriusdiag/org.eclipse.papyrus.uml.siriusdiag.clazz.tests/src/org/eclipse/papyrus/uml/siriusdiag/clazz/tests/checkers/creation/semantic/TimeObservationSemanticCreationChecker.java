...
/**
 * Creation checker for UML TimeObservation
 */
public class TimeObservationSemanticCreationChecker extends AbstractSemanticNodeCreationChecker {
	 ...
	 * @param expectedOwner
	 */
	public TimeObservationSemanticCreationChecker(final EObject expectedOwner) {
		super(expectedOwner);
	}
	...
	protected void validateSemanticElementInstance(final EObject semanticElement) {
		Assert.assertTrue(NLS.bind("The created element must be a UML TimeObservation instead of a {0}.", semanticElement.eClass().getName()), semanticElement instanceof TimeObservation); //$NON-NLS-1$
	}
