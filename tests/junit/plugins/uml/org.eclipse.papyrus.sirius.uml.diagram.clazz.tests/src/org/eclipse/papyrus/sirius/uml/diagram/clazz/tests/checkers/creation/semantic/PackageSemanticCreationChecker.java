...
/**
 * Creation checker for UML Package
 */
public class PackageSemanticCreationChecker extends AbstractSemanticNodeCreationChecker {
	 ...
	 * @param expectedOwner
	 */
	public PackageSemanticCreationChecker(final EObject expectedOwner) {
		super(expectedOwner);
	}
	...
	protected void validateSemanticElementInstance(final EObject semanticElement) {
		Assert.assertTrue(NLS.bind("The created element must be a UML Package instead of a {0}.", semanticElement.eClass().getName()), semanticElement instanceof Package); //$NON-NLS-1$
	}
