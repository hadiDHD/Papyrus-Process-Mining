...
/**
 * Creation checker for UML Model
 */
public class ModelSemanticCreationChecker extends AbstractSemanticNodeCreationChecker {
	 ...
	 * @param expectedOwner
	 */
	public ModelSemanticCreationChecker(final EObject expectedOwner) {
		super(expectedOwner);
	}
	...
	protected void validateSemanticElementInstance(final EObject semanticElement) {
		Assert.assertTrue(NLS.bind("The created element must be a UML Model instead of a {0}.", semanticElement.eClass().getName()), semanticElement instanceof Model); //$NON-NLS-1$
	}
