...
/**
 * Creation checker for UML Component
 */
public class ComponentSemanticCreationChecker extends AbstractSemanticNodeCreationChecker {
	 ...
	 * @param expectedOwner
	 */
	public ComponentSemanticCreationChecker(final EObject expectedOwner) {
		super(expectedOwner);
	}
	...
	protected void validateSemanticElementInstance(final EObject semanticElement) {
		Assert.assertTrue(NLS.bind("The created element must be a UML Component instead of a {0}.", semanticElement.eClass().getName()), semanticElement instanceof Component); //$NON-NLS-1$
	}
