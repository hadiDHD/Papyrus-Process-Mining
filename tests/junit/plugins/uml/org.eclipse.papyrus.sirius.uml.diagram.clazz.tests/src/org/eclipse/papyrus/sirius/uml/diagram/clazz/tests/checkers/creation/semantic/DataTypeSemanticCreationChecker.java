...
/**
 * Creation checker for UML DataType
 */
public class DataTypeSemanticCreationChecker extends AbstractSemanticNodeCreationChecker {
	 ...
	 * @param expectedOwner
	 */
	public DataTypeSemanticCreationChecker(final EObject expectedOwner) {
		super(expectedOwner);
	}
	...
	protected void validateSemanticElementInstance(final EObject semanticElement) {
		Assert.assertTrue(NLS.bind("The created element must be a UML DataType instead of a {0}.", semanticElement.eClass().getName()), semanticElement instanceof DataType); //$NON-NLS-1$
	}
