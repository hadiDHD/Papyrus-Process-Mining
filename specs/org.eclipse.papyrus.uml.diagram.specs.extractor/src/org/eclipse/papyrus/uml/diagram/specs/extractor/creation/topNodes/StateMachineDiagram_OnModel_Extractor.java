 ...
 * 
 */
@PluginResource("resources/creation/StateMachineDiagram/StateMachineDiagram_OnModel.di")
public class StateMachineDiagram_OnModel_Extractor extends AbtractPapyrusDiagramSpecificationsExtractor {

	// just here to force dependency
	@SuppressWarnings("unused")
	private final UmlStateMachineDiagramForMultiEditor editor = null;

	/**
	 * The URI of the palette definition
	 * 
	 */
	private final URI PALETTE_DEFINITION = URI.createPlatformPluginURI("org.eclipse.papyrus.uml.diagram.statemachine/model/PapyrusUMLStateMachineDiagram.paletteconfiguration", false); //$NON-NLS-1$

	private static final String EMPTY__DIAGRAM_NAME = "StateMachineDiagram_Empty"; //$NON-NLS-1$

	private static final String ALL_ELEMENTS__DIAGRAM_NAME = "StateMachineDiagram_AllElements"; //$NON-NLS-1$

	private static final String FILE_NAME = "StateMachineDiagram_Requirements";//$NON-NLS-1$
	...
	protected String getMetaclassFromEditPart(EditPart ep) {
		...
		if (metaclass == null) {
			if (ep instanceof ContextLinkAppliedStereotypeEditPart) {
				return "ContextLink"; //$NON-NLS-1$
			}
		}
		return metaclass;
	}
	...
	/**
	 * A custom Palette reader with he ElementType for StateMachine!
	 */
	private class CustomPaletteDescriptionReader extends PaletteDescriptionReader {

		private String STATEMACHINE_ELEMENT_TYPE = "org.eclipse.papyrus.umldi.StateMachine_Shape"; //$NON-NLS-1$

		private String STATEMACHINE = "StateMachine"; //$NON-NLS-1$
		...
		public CustomPaletteDescriptionReader(URI paletteURI) {
			...
			list.add(STATEMACHINE_ELEMENT_TYPE);
			nodesMetaclassesVSelementTypesID.put(STATEMACHINE, list);

			// nodesByPaletteToolID //not in the palette
			elementTypesVSMetaclass.put(STATEMACHINE_ELEMENT_TYPE, STATEMACHINE);
	...
	protected RequirementsExcelWriter createRequirementsExcelWriter(String filePath, PaletteDescriptionReader paletteReader) {
		return new RequirementsExcelWriter(filePath, paletteReader) {
			...
			protected XSSFSheet writePaletteSheet(XSSFWorkbook workbook) {
				...
				cell.setCellValue("The StateMachine element is not proposed in this Papyrus Diagram Palette. We add it to get a working specifications extractor!"); //$NON-NLS-1$
				sheet.addMergedRegion(new CellRangeAddress(rowIndex, rowIndex, 0, 2));
				return sheet;
			}
